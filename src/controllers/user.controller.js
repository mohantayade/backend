...
const updateAccountDetails = asyncHandler(async(req,res)=>{
  ...
  const user = await User.findByIdAndUpdate(
    ...
    return res.status(200).json(new ApiResponce(200,user,"Account details update Successfully"))
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
    ...
    const user = await User.findByIdAndUpdate(req.user?._id,
      ...
      //***/ image delete option not createted it importent so take it
})
...
const getUserChannelProfile = asyncHandler(async(req,res)=>{
  ...
  },
  {
    $project:{
     ...
     return res.status(200).json(new ApiResponce(200, channel[0],"user channel fetched Successfully"))
})
